...
- `単一銘柄リプレイ`: 1銘柄を指定し、その銘柄だけで過去の買いポイントと成績を確認します。
- `watchlist過去スキャン再現`: `watchlist.csv`全体を対象に、過去の各スキャン時刻で当時の買い候補を再現します。実際の`market_runner.py`に近い検証です。
...
1. `過去リプレイ検証`タブを開きます。
2. モードで`単一銘柄リプレイ`を選びます。
3. 銘柄コード、期間、時間足を選び、`過去データ取得`を押します。
4. 取得データ件数、最初の日時、最後の日時を確認します。
5. 開始日時、終了日時、最小スコア、対象ルール、最大仮想買い件数を設定します。
6. `リプレイ検証を実行`を押します。
7. 勝率、平均リターン、最大利益率平均、最大下落率平均、損切り到達件数、利確到達件数を確認します。
8. チャートでは終値、仮想買いポイント、損切りライン、利確ラインを確認できます。

watchlist過去スキャン再現の使い方:

1. `過去リプレイ検証`タブを開きます。
2. モードで`watchlist過去スキャン再現`を選びます。
3. `watchlistから使う最大銘柄数`、期間、時間足、開始日、終了日、スキャン間隔を選びます。
4. 最小スコア、対象、1回の最大仮想買い件数、同一銘柄の再シグナル抑制、想定株数を設定します。
5. 必要なら`日足スコア上位N銘柄に絞る`、`日足スコア上位N`、`日足スコアの最低点`を設定します。
6. 必要なら`対象銘柄のみ`、`除外銘柄`に銘柄コードを入力します。
7. `過去スキャン再現を実行`を押します。
8. 全体サマリー、銘柄別サマリー、トレード一覧、ランキング、累計損益カーブを確認します。
9. 各スキャン時刻では、その時刻以前のOHLCVだけを買い判定に使います。未来データは結果検証だけに使います。

日足上位Nの意味:

- `watchlistから使う最大銘柄数`は、最初に`watchlist.csv`から読み込む銘柄数の上限です。
- `日足スコア上位N`は、各スキャン時刻で日足スコア順に残す銘柄数です。
- `1回の最大仮想買い件数`は、その時刻で実際に仮想買いとして採用する件数です。
- 処理順は、watchlist読み込み → 対象/除外フィルター → watchlist上限 → 日足スコア上位N → 5分足エントリー → リスク条件 → 最大仮想買い件数、です。
- MVPの日足スコアは、25日線より上、前日終値より上、直近5日高値圏、日足出来高増加の4条件のOK率で計算します。
- 通常スキャンでも、日足スコア上位10銘柄、最低70点を5分足判定の母集団にします。

買い条件表示の見方:

- `日足順位`は、そのスキャン時刻で日足スコア順に何位だったかを表します。
- `日足スコア`は日足条件の強さです。3/4 OKなら75点、4/4 OKなら100点です。
- `日足条件`、`5分足条件`、`リスク条件`が揃うほど、ルール上の買い候補に近づきます。
- `日足上位N`は母集団フィルターであり、最終的な仮想買いは5分足条件とリスク条件も通過した銘柄だけです。
- この機能は過去検証とpaper trading向けです。実売買、発注、自動売買ではありません。

対象銘柄指定と除外銘柄指定:

- `対象銘柄のみ`が空欄なら、`watchlist.csv`全体を対象にします。
- `対象銘柄のみ`に`6501, 6723.T, 6752.T`のように入れると、その銘柄だけで検証します。
- `除外銘柄`に`6501.T, 7751.T`のように入れると、検証対象から一時的に外します。
- 入力はカンマ区切り、スペース区切り、改行区切りに対応しています。
- `6501`、`6501.T`、`6501.t`はいずれも`6501.T`として扱います。
- `対象銘柄のみ`と`除外銘柄`の両方に同じ銘柄がある場合は、除外を優先します。
- watchlistに存在しない銘柄コードは画面に警告表示し、存在する銘柄だけで続行します。
- 対象が0件になった場合は、検証を実行しません。
- `watchlist.csv`を直接編集せずに、負け銘柄TOP5を一時除外して再検証したり、半導体、電力、データセンターなどテーマ別の銘柄だけを指定して比較検証できます。

watchlist過去スキャン再現の注意:

- watchlist全体から、当時スコア条件を満たした買い候補を抽出します。事前にユーザーが選んだ1銘柄だけを見る検証ではありません。
- 同一銘柄の再シグナル抑制を使い、30分、1時間、3時間、当日中の重複を抑えられます。
- 取得失敗銘柄があっても全体処理は継続し、詳細デバッグ情報に理由を表示します。
- 5分足でwatchlist全件を処理すると重くなる場合があります。最初は10〜30銘柄、1ヶ月、5分足または15分足を推奨します。
- 保存結果は`replay_trades.db`に`replay_mode=watchlist_scan`として保存され、単一銘柄リプレイと区別できます。
...
画面上部の`判定ロジック確認`では、現在の買い候補判定がどの時間足と条件を使っているかを確認できます。

通常株価スキャン:

- `株価スキャンを実行`は、日足フィルター + 5分足エントリーのマルチ時間足判定を使います。
- まず日足データで、25日線より上、前日終値より上、直近5日高値圏、日足出来高増加を確認します。
- 次に、取得できる場合は当日5分足でVWAP上、直近高値突破、押し目反発、出来高急増を確認します。
- 日足3/4以上 + 5分足2/4以上なら`買い候補`、日足3/4以上で5分足未達なら`監視`、日足未達なら`触らない`として扱います。
- 市場時間外などで5分足が取れない場合はアプリを落とさず、日足通過なら`監視`として表示します。

場中エントリー監視:

- `場中エントリー監視`は、現状では5分足を中心に使います。
- 5本、10本、20本の5分足移動平均、安値切り上げ、節目突破/維持、直近12本高値圏、直近平均出来高比、RSI、損切り幅、R/Rを見ます。
- このタブ単体の既存監視ロジックでは、VWAPはまだ計算していません。
- 通常株価スキャンのマルチ時間足判定とは別の場中監視表示です。

過去スキャン再現:

- `マルチ時間足判定を使う`がONの場合、6か月日足と選択時間足を組み合わせて判定します。
- 選択した時間足（5分足、15分足、1時間足、日足）で、各スキャン時刻以前のローソク足だけを使って買い判定します。
- 日足フィルターは、前日までの確定日足 + その時刻までの5分足から作った当日途中の日足で評価します。
- 未来データは買い判定には使わず、利確、損切り、期限到達などの結果検証だけに使います。
- 初期値は日足3/4以上、5分足2/4以上、VWAP条件ON、出来高急増条件ONです。

各買い候補カードには`判定内訳`を表示します。

- 日足フィルター: 25日線より上、前日終値より上、直近5日高値圏、出来高増加
- 5分足エントリー: VWAP上、直近高値突破、押し目反発、出来高急増
- 通常株価スキャンのカードでは、日足OK数、5分足OK数、統合判定を表示します。
- 場中エントリー監視のカードでは、既存の場中監視結果とあわせて内訳を確認できます。

## マルチ時間足判定

マルチ時間足判定は、日足で強い銘柄だけを対象にし、5分足で実際のエントリー形を確認するためのpaper trading向け判定です。実売買、発注、自動売買ではありません。OpenAI APIも使いません。

日足フィルターの4条件:

- 25日線より上: `close > 25日移動平均`
- 前日終値より上: `current_close > previous_daily_close`
- 直近5日高値圏: `current_close >= recent_5day_high * 0.98`
- 日足出来高増加: `current_volume >= 5日平均出来高 * 1.2`

5分足エントリーの4条件:

- VWAP上: 当日5分足から計算したVWAPより現在値が上
- 直近高値突破: 現在足を除いた過去12本高値を現在値が突破
- 押し目反発: VWAP付近まで押したあと、現在値がVWAPと前足終値を上回る
- 出来高急増: 現在足出来高が過去12本平均の1.5倍以上
